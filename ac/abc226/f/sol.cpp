...
int main() {
  ...
  int N, K;
  cin >> N >> K;
  map<pair<int, long long>, Mint> ct;
  ct[{0, 1}] = 1;
  for (int i = 1; i <= N; ++i) {
    auto old_ct = ct;
    for (auto e : old_ct) {
      int c = e.first.first;
      long long g = e.first.second;
      Mint r = e.second;
      long long x = lcm(g, i);
      int choose = 0;
      Mint mult = 1;
      int left = N - c;
      for (int j = i; j + c <= N; j += i) {
        choose += 1;
        mult *= C(left, i) * fact(i - 1); 
        //debug(i, choose, mult, j + c, x, r * mult / fact(choose));
        ct[{j + c, x}] += r * mult / fact(choose);   
        left -= i;
      }
    }
  }
