...
int main() {
  ...
  function<void(int)> Dfs = [&](int v) {
    for (auto u : g[v]) {
      ...
      debug(dp[v], dp[u]);
      int size = int(dp[u].size());
      vector<Mint> pref(size);
      for (int i = 0; i < size; ++i) {
        pref[i] = pref[i - !!i] + dp[u][size - i - 1];
      }  

      int s = int(dp[v].size());
      vector<Mint> new_dp(s + size);
      for (int i = 0; i < s; ++i) {
        for (int j = 0; j < size; ++j) {
          int pre_v = i, next_v = s - i - 1;
          int pre_u = j + 1, next_u = size - j - 1;
          new_dp[i + j + 1] += dp[v][i] * pref[j] * C(pre_v + pre_u, pre_v) * C(next_v + next_u, next_v);
        }
      }
      debug(new_dp);
      swap(dp[v], new_dp);
  ...
  Dfs(0);
  cout << accumulate(dp[0].begin(), dp[0].end(), Mint(0)) * 2 << '\n';
}